...
namespace ImTricks {
	...
	namespace Animations {
		...
		float FastFloatLerp(const char* label, int seed, bool state, float min, float max, float speed) {
			...
			if (value == valuesMapFloat.end()) {
				valuesMapFloat.insert({ id, min });
				value = valuesMapFloat.find(id);
			}

			const float frameRateSpeed = speed * (1.f - ImGui::GetIO().DeltaTime);

			value->second = ImLerp( value->second, state ? max : min, frameRateSpeed );
	...
	namespace NotifyManager {
		...
		void AddNotify(const char* message, NotifyState state) {
			notifies.push_back({ message, state, GetTickCount64() + 6000 });
		}
		...
		void HandleNotifies(ImDrawList* draw) {
			...
			ImVec2 NotifyPos = ImVec2(ScreenSize.x - 310, 30.f);

			auto DrawNotify = [&draw, &NotifyPos](NotifyStruct notify) {

				const auto NotifyEndPos = NotifyPos + ImVec2(280, 50);

				float alpha = ( ( 6000 - ( notify.time - GetTickCount64( ) ) ) * 6 ) / 6000.f < 4.f ? ( ( 6000 - ( notify.time - GetTickCount64( ) ) ) * 6 ) / 2000.f : ( ( notify.time - GetTickCount64( ) ) * 6 ) / 2000.f;

				draw->AddRectFilled( NotifyPos, NotifyEndPos, ImGui::GetColorU32( ImGuiCol_FrameBg, alpha ), ImGui::GetStyle( ).FrameRounding );

				auto StateColor = ImColor(45, 45, 45);

				switch (notify.state) {
				case ImTrickNotify_Success: StateColor = ImGui::GetColorU32( ImGuiCol_Scheme, alpha ); break;
				case ImTrickNotify_Warning: StateColor = ImColor( 255 / 255.f, 131 / 255.f, 17 / 255.f, alpha ); break;
				case ImTrickNotify_Danger: StateColor = ImColor( 255 / 255.f, 17 / 255.f, 17 / 255.f, alpha ); break;
				...
				draw->AddRectFilled( NotifyPos, ImVec2( NotifyPos.x + 50, NotifyEndPos.y ), StateColor, ImGui::GetStyle( ).FrameRounding, ImDrawCornerFlags_Left );
				draw->AddText( { NotifyPos.x + 70, NotifyPos.y + 25 - ImGui::CalcTextSize( notify.message ).y / 2 }, ImGui::GetColorU32( ImGuiCol_Text, alpha ), notify.message );

				switch ( notify.state ) {
				
				case ImTrickNotify_Success:
				ImGui::RenderCheckMark( draw, { NotifyPos.x + 18, NotifyPos.y + 17 }, ImGui::GetColorU32( ImGuiCol_CheckMark, alpha ), 15.f );
				break;
				case ImTrickNotify_Danger:
				draw->AddLine( { NotifyPos.x + 25 - 5, NotifyPos.y + 25 - 5 }, { NotifyPos.x + 25 + 5, NotifyPos.y + 25 + 5 }, ImGui::GetColorU32( ImGuiCol_CheckMark, alpha ) );
				draw->AddLine( { NotifyPos.x + 25 + 5, NotifyPos.y + 25 - 5 }, { NotifyPos.x + 25 - 5, NotifyPos.y + 25 + 5}, ImGui::GetColorU32( ImGuiCol_CheckMark, alpha ) );
				break;
